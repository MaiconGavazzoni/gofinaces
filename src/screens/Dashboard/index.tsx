...
export function Dashboard() {
  ...
  async function loadTransaction() {
    ...
    const transactionsFormatted: DataListProps[] = transactions.map((item: DataListProps) => {
      ...
      const amount = Number(item.amount).toLocaleString('pt-Br', { style: 'currency', currency: 'BRL' }); //formatação de moeda
      const date = Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit' }).format(new Date(item.date)); //formatação de datas
      return {
        id: item.id,
        name: item.name,
        amount: amount,
    ...
    setHighlightData({
      ...
      expensives: {
        ...
        lastTransaction : lastTransactionExpensives === 0 ? 'Não há saídas' :`Última saída ${lastTransactionExpensives}`
      },
      total: {
